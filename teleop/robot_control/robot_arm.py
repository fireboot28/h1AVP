...
class H1ArmController:
    def __init__(self):
        ...
        self.__packFmtHGLowCmd =  '<4B4IH2x' + 'B3x5f3I' * kNumMotors + '4B' + '55Bx2I'
        #'<2B2x' + ('B3x5f3I' * kNumMotors) + '2I'#'<2B2x' + 'B3x5fI' * 35 + '5I'
    ...
    def __pack_crc(self):
        ...
        for i in range(kNumMotors):
            ...
            origData.extend(self.msg.motor_cmd[i].reserve) #Was .append
        ...
        origData.append(self.msg.reserve) #.extend(reserve) --reserve is an array in hg
...
# # from unitree_sdk2py.comm.motion_switcher.motion_switcher_client import MotionSwitcherClien
# import struct
# from enum import IntEnum
# import copy
# import numpy as np
...
#         # Initialize the Channel factory first
...
#         while not self.lowstate_subscriber.has_data():
#             print("lowstate_subscriber is not ok! Please check SDK.")
#             time.sleep(0.01)
        
#         # Assuming lowstate_subscriber.get_data() retrieves latest data
#         lowstate_data = self.lowstate_subscriber.get_data()
#         for id in JointIndex:
#             self.msg.motor_cmd[id].q = lowstate_data.motor_state[id].q
#             self.q_target.append(self.msg.motor_cmd[id].q)
...
#         # Initialize motors
#         duration = 1000
#         init_q = np.array([lowstate_data.motor_state[id].q for id in JointIndex])
#         print("Lock Leg...")
#         for step in range(duration):
#             time.sleep(0.001)
#             q_t = init_q + (self.q_target - init_q) * step / duration
#             for i, id in enumerate(JointIndex):
#                 self.msg.motor_cmd[id].mode = 1
#                 if id not in JointArmIndex:
#                     self.msg.motor_cmd[id].kp = 200
...
#             self.pre_communication()
#             self.lowcmd_publisher.write(self.msg)  # Assuming .write(msg) publishes the message

#         print("Lock Leg OK!")

#         # Start threads as before
#         threading.Thread(target=self.SubscribeState).start()
#         threading.Thread(target=self.Control).start()
#         threading.Thread(target=self.LowCommandWriter).start()
...
#         # Updated CRC computation
#         crc_calc = CRC()
#         self.msg.crc = crc_calc.crc32(calcdata)
